# --- inference_2d/amateur_inference.py ---
import os
import sys
...
MODAL_LORA_PATH = "/vol/models/model/amateur"
LOCAL_LORA_PATH = "/home/aikusrv04/Doodle/FINAL/model/amateur"
...
SEED = 42
PROMPT_PREFIX = "a childlike crayon drawing"
...
OUTPUT_DIR = "/home/aikusrv04/Doodle/FINAL/outputs_2d/amateur"


def load_amateur_pipeline():
    ...
    if os.path.exists(LORA_WEIGHTS_PATH):
        ...
        print("[WARN] Amateur LoRA not found. Using base model.")
...
def run_amateur_inference(pipe, image: Image.Image, user_prompt: str) -> Image.Image:
    prompt = f"{PROMPT_PREFIX}, cute {user_prompt} character, no background"
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python amateur_inference.py <NUM> <USER_PROMPT>")
        print("Example: python amateur_inference.py 7 \"blue dragon\"")
        sys.exit(1)

    num = str(sys.argv[1])
    user_prompt = " ".join(sys.argv[2:])  # 공백 포함 프롬프트
